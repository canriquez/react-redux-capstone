...
const RenderDashBoard = ({
  ...
  const handleKeySearch = inputValue => {
    ...
    keysearch = state.crypto.map((obj, idpos) => ({ ...obj, idpos })).filter(asset => (asset.name.substr(0, inputValue.length).toUpperCase() == inputValue.toUpperCase())).map(obj => ({ name: obj.name, idpage: (Math.trunc(obj.idpos / 5) + 1) }));
